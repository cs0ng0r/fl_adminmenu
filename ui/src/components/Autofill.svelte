...
<div
	class="w-[22vh] flex flex-col bg-secondary rounded-[0.5vh] border-[0.1vh] border-primary"
>
	...
	{#if DataDropdownActive}
		...
		>
			...
			{:else if data === 'vehicles'}
				{#each $VEHICLE_DATA.filter((i) => i.name
							.toLowerCase()
							.includes(search.toLowerCase()) || i.model
							.toLowerCase()
							.includes(search.toLowerCase())) as i}
					<button
						class="w-full p-[0.5vh] flex justify-start text-start px-[1vh] gap-[0.5vh] hover:bg-tertiary"
						on:click={() => selectData(i.name, i.model)}
					>
						<p>{i.name}</p>
						<p>({i.model})</p>
					</button>
				{/each}
			{:else if data === 'items'}
				{#each $ITEM_DATA.filter((i) => i.label
							.toLowerCase()
							.includes(search.toLowerCase()) || i.value
							.toLowerCase()
							.includes(search.toLowerCase())) as i}
					<button
						class="w-full p-[0.5vh] flex justify-start text-start px-[1vh] gap-[0.5vh] hover:bg-tertiary"
				...
				{/each}
			{:else if data === 'jobs'}
				{#each $JOB_DATA.filter((i) => i.label
							.toLowerCase()
							.includes(search.toLowerCase()) || i.value
							.toLowerCase()
							.includes(search.toLowerCase())) as i}
			...
			{:else if data === 'locations'}
				{#each $LOCATION_DATA.filter((i) => i.label
							.toLowerCase()
							.includes(search.toLowerCase()) || i.value
							.toLowerCase()
							.includes(search.toLowerCase())) as i}
			...
			{:else if data === 'pedlist'}
				{#each $PED_LIST.filter((i) => i.label
							.toLowerCase()
							.includes(search.toLowerCase()) || i.value
							.toLowerCase()
							.includes(search.toLowerCase())) as i}
